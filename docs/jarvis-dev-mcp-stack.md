# Jarvis-Dev: Stack de MCPs Recomendados

## Critério de Seleção

Cada MCP foi avaliado por 3 eixos específicos do Jarvis-Dev:

| Eixo | Pergunta |
|---|---|
| **Voice-First Utility** | O resultado é *falável*? Dá pra resumir em 10 segundos de áudio? |
| **Autonomia** | Reduz quantas vezes o Jarvis precisa perguntar ao usuário? |
...
**Por que é essencial:** O Jarvis precisa de Git como um sentido primário. Sem ele, todo git passa pelo `execute_shell`, sem parsing estruturado.
...
**Por que é essencial:** O Jarvis está gerando prompts para o AGY criar código. Se ele alucina uma API, o código quebra. Context7 elimina isso.
...
**Por que é essencial:** Voz é linear — o usuário não pode voltar e reler. O Jarvis precisa pensar estruturadamente *antes* de falar pra não dar respostas confusas em decisões arquiteturais complexas.
...
**Por que é alto valor:** O Jarvis pode abrir PR, responder review comments e checar CI/CD sem sair do fluxo de voz. Essencial pra quem trabalha em equipe.
...
**Por que é alto valor:** O Jarvis precisa pesquisar errors, comparar libs, e encontrar soluções sem que o usuário abra o navegador (impossível, tela no bolso).

**Exemplo de voz:**
> 👤 *"Jarvis, tô com esse erro: CORS policy blocked. O que pode ser?"*
> 🤖 *(busca no Brave)*
...
**Por que é útil:** O Jarvis vira PM. *"Cria uma task pra corrigir o bug do login, prioridade alta."* → Ticket criado no board enquanto você caminha.
...
**Por que é útil:** Quando a voz não é possível (reunião, ônibus lotado), o Telegram vira fallback. O usuário digita no Telegram, o Jarvis responde lá E atualiza o estado da sessão de voz.
...
```bash
#!/bin/bash
# jarvis-mcp-setup.sh — Instala todos os MCPs do Jarvis-Dev
...
> [!TIP]
> **Ordem de impacto no Jarvis-Dev por voz:**
